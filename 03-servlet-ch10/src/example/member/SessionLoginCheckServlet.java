...
public class SessionLoginCheckServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 세선 객체를 getSession() 메소드를 호출해서 가져온다.
		HttpSession session = req.getSession();

		/*
		 * getAttribute 메서드를 이용하여 key를 이용해 해당 값을 가져온다.
		...
		// 삼항 연산자를 이용해서 memberId값이 null 이면 로그인이 안된 상태로 판단한다.
		boolean isLogin = memberId == null ? false : true;
		if (isLogin) {
			out.println("아이디 '" + memberId + "'로 로그인 한 상태얌<br>");
			out.println("<a href='session-logout.do'>로그아웃</a>");

		} else {
			out.println("로그인하지 않은 상태얌");
		}
