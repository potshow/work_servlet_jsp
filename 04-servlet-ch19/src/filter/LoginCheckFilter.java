...
public class LoginCheckFilter implements Filter{
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
		...
		// HttpServletRequest의 기능을 사용하기 위해 다운캐스팅 함. (getSession 메서드)
		// SevletRequest가 부모 클래스임.
		HttpServletRequest httpReq = (HttpServletRequest) req;
		
		// getSession(false) : 현재 세션이 존재하면 세션 객체를 리턴, 아니면 null값 리턴 (login이 되있는지 안되있는지 확인만 해주는 것)
		// getSession(true) : 현재 세션이 존재하면 세션 객체를 리턴, 아니면 새로 세션을 생성해서 리턴.
		HttpSession session = httpReq.getSession(false);
		
		// 로그인 여부를 판단하는 boolean 변수 
		boolean login = false;
		// 세션이 존재하면
		if (session != null) {
